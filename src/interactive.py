...
if __name__ ==  '__main__':
    ...
    GPIO.setup(Rotaryknob.PIN_CLK, GPIO.IN, pull_up_down = GPIO.PUD_UP)
    GPIO.setup(Rotaryknob.PIN_DT, GPIO.IN, pull_up_down = GPIO.PUD_UP)
    GPIO.setup(Rotaryknob.BUTTON_PIN, GPIO.IN, pull_up_down = GPIO.PUD_UP)

    rotary.set_clk_last(GPIO.input(Rotaryknob.PIN_CLK))
    ...
    GPIO.add_event_detect(Rotaryknob.PIN_CLK, GPIO.BOTH, callback=rotary.turn_knob, bouncetime=50)
    GPIO.add_event_detect(Rotaryknob.BUTTON_PIN, GPIO.FALLING, callback=rotary.counter_reset, bouncetime=50)
